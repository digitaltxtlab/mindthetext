...
sent_plot_data{1} = [yr,smooth(y_bar(:,1)-nanmean(y_bar(:,1))),smooth(y_bar(:,2))/5];    
% vectors
x = meta_av_mat(:,3);
idx = isnan(x); x(idx) = [];
...
%h4 = errorbar(yr,smooth(y_bar(:,1)),y_bar(:,2)/5);
    ...
    %ylim([.2 .5])
    set(gca,'xtick',linspace(min(yr),max(yr),5))
sent_plot_data{2} = [yr,ytmp,e];
    figSize(f,10,23)
plotCorrect
%save(strcat(dd,'/sent_vects.mat'),'a_sent','a_sent_bar','g_sent','g_sent_bar')
saveFig('/home/kln/Documents/ms/mind_text/figures/fig_sent.png')
% save('data/plot_data.mat','sent_plot_data','-append');
